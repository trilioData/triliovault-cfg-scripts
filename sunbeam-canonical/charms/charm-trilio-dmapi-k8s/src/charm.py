...
CONTAINER = "trilio-dmapi"
CONFIG_PATH = "/etc/dmapi/dmapi.conf"
...
class TrilioDmapiK8sCharm(ops.CharmBase):
    ...
    def __init__(self, *args):
        super().__init__(*args)
        self.framework.observe(self.on.trilio_dmapi_pebble_ready, self._on_pebble_ready)
    ...
    def _write_config(self, container):
        ...
        db_url = (
            f"mysql+pymysql://{db.get('username', 'dmapi')}:{db['password']}"
            f"@{db['host']}/{db.get('database', 'dmapi')}"
        )
        auth_url = (
    ...
    def _update_pebble_layer(self, container):
        layer = ops.pebble.Layer({
            ...
            "services": {
                "dmapi": {
                    ...
                    "command": f"/usr/bin/dmapi --config-file {CONFIG_PATH}",
                    "startup": "enabled",
                }
            },
        })
        ...
        container.replan()
        logger.info("Pebble layer applied for dmapi")
