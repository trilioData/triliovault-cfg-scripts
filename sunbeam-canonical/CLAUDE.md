...
- `trilio-wlm` — **file search** needs `/dev/nbd*`. See the next section.
...
This is the trap: `/dev` is per-container, but `lsmod` and `/sys/block` are not. A container can show `nbd` loaded via `lsmod` and 128 entries under `/sys/block/nbd*` while holding **zero** `/dev/nbd*` nodes, because containerd gives an unprivileged container a minimal private devtmpfs. Loading the module from the privileged `trilio-dms` sidecar (`_load_nbd_module`) shares the *module*, never the device nodes. When this happens `find_available()` returns `[]`, `connect()` returns the boolean `False`, and the search dies running `lsblk -O -J /dev/False` — surfacing in Horizon as "Invalid state: Error in searching files, Contact your administrator", with the real clue only in `/var/log/triliovault/workloadmgr-filesearch.log`. The `modprobe: FATAL: Module nbd not found` line in that log is a red herring: `filesearch.py` wraps that call in try/except and only logs it.

`privileged: true` is required, not merely convenient: it supplies the host devtmpfs (so the `/dev/nbdXpN` partition nodes `partprobe` creates *at runtime* appear too — static hostPath `CharDevice` mounts cannot, since those nodes do not exist at pod creation time) and the `CAP_SYS_ADMIN` that `qemu-nbd -c`, `partprobe`, the LVM scans and the partition `mount` calls all require. Every other platform already does this — on Kolla (`kolla-ansible/ansible/roles/triliovault/defaults/main.yml`) all four wlm containers run `privileged: True` with `/dev:/dev` and `/lib/modules:/lib/modules:ro`.

**Related caveat:** `_load_nbd_module` runs `modprobe nbd nbds_max=128`, but modprobe is a no-op on an already-loaded module — it does **not** re-apply `nbds_max`. On a node where something else loaded `nbd` first, the count stays at the default 16, capping how many backup images one file search can attach concurrently.
