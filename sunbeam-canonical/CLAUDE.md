...
3. **`No cloud admin trust found. Please recreate using CLI`** (from `backup-target-create`, even right after a successful `trust-create`) — `workloadmgr/compute/nova.py`'s `_get_trusts('cloud_admin', CONF.cloud_admin_project_id)` looks up the trust by the charm config options `cloud-admin-user-id`/`cloud-admin-project-id`, which default to empty strings and are **not** auto-populated from the DB despite what their `charmcraft.yaml` description implies ("Leave empty if using trust-create action (WLM reads from DB after trust creation)" — this is aspirational/inaccurate for the currently-installed `workloadmgr` version). Must be set via `juju config trilio-wlm-k8s cloud-admin-user-id=<id> cloud-admin-project-id=<id>` to the same Keystone user/project used when running `trust-create`.
...
**This is not a one-time setup step — verify `cloud-admin-user-id`/`cloud-admin-project-id` any time WLM is redeployed or its config is reset.** Confirmed on 2026-07-24: this environment had lost/never had these two config values set, causing every `workload-create` to fail with "No cloud admin trust found" despite a genuinely valid trust already existing in Keystone. Fixed by `juju config trilio-wlm-k8s cloud-admin-user-id=<real Keystone admin user UUID> cloud-admin-project-id=<real Keystone admin project UUID>` (get both via `openstack user show admin -f value -c id` / `openstack project show admin -f value -c id`), then restarting the wlm-api/wlm-workloads/wlm-cron/wlm-scheduler Pebble services. Symptom to watch for: `trust-create` itself also fails with the identical "No cloud admin trust found" message (it calls the same lookup as a post-creation side effect to trigger the default backup-target mount) — don't mistake this for the trust-create having failed; check `workloadmgr trust-list`/`setting-list` directly against Keystone (`openstack trust list`) to confirm the trust object actually exists before concluding otherwise.

**Known unresolved product bug downstream of a working cloud-admin trust — TVAULT-7514**: even with the trust fully fixed, `workload-snapshot` fails immediately with `Failed creating workload snapshot: The service catalog is empty.` Root-caused (confirmed directly against Keystone, independent of WLM) to `workloadmgr/compute/nova.py`'s `nova_micro_client()` — its no-password fallback branch wraps an already trust-scoped token in a `v3.Token()` auth plugin to build a fresh client session, which Keystone explicitly rejects (403 "Using trust-scoped token to create another token"); keystoneauth1 surfaces that rejection further up the stack as the misleading `EmptyCatalog` error. This blocks ALL snapshot creation (full and incremental, any backup target type) on ANY deployment platform once a cloud-admin trust is otherwise correctly configured — it is not Sunbeam-specific. Needs a core `workloadmgr` code fix, not a charm/config workaround; tracked as TVAULT-7514, blocking TVAULT-7404.
