...
**The Trilio debs ship their own competing units, and they must be masked (TVAULT-7654).** `python3-trilio-dms` ships `trilio-dms-server.service` and `python3-tvault-contego` ships `tvault-contego.service`, both enabled and started by their postinst. This does **not** collide on other distros — everywhere else these services run in a container, and the classic Canonical charm (`juju-charms/charm-trilio-data-mover`) deliberately *uses* the packaged units rather than writing its own. Sunbeam is the only deployment that both installs the debs on a systemd host **and** writes units of its own, so it is the only one that ends up with two.
