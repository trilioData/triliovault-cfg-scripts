...
**Next blocker after both of the above are fixed — TVAULT-7515 (open, unresolved):** snapshot execution now reaches the freeze/snapshot step, then fails with `novaclient.exceptions.ClientException: Unknown Error (HTTP 502)` from contego's `vast_instance` Nova server-action call. The request never appears in nova-api's own request log at all (ordinary GET calls to the same Nova endpoint succeed and log normally in the same window) and nova-compute on the correct compute host shows nothing either — the request is failing before it ever reaches Nova's application code. `python3-tvault-contego` (the compute-side counterpart) is confirmed installed at the expected version. Not yet root-caused; likely somewhere in the ingress/proxy path for this specific POST server-action call, or in how Nova recognizes the custom action. This is the current active blocker for finishing NFS/S3 backup validation.
