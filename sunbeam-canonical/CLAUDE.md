...
The `trilio-data-mover` charm writes both units to `/lib/systemd/system/` during install:
...
**The Trilio debs ship their own competing units, and they must be masked (TVAULT-7654).** `python3-trilio-dms` ships `trilio-dms-server.service`, which its postinst enables and starts. `python3-tvault-contego` ships `tvault-contego.service` — the unit the classic Canonical charm manages — which has so far come out *disabled* on Sunbeam; it is masked anyway, because nothing guarantees that and the failure mode would be identical. This does **not** collide on other distros — everywhere else these services run in a container, and the classic Canonical charm (`juju-charms/charm-trilio-data-mover`) deliberately *uses* the packaged units rather than writing its own. Sunbeam is the only deployment that both installs the debs on a systemd host **and** writes units of its own, so it is the only one that ends up with two.

They are not harmlessly idle. The packaged DMS unit's `ExecStart=/usr/bin/trilio-dms-server` passes no `--config-file`, and `trilio_dms.config` falls back to `SERVER_CONFIG_FILE = /etc/triliovault-dms/server.conf` — the same file the charm renders. Both processes therefore join the **same RabbitMQ queue under the same `node_id`**, and RabbitMQ round-robins mount/unmount requests between them. The packaged copy runs as `nova` without the snap `PYTHONPATH` and can service neither: mounts complete `status=error` having mounted nothing, and unmounts fail to kill the root-owned `s3vaultfuse` (`[Errno 1] Operation not permitted`) but still `umount` and delete the mount directory — pulling it out from under an in-flight snapshot. Both paths reach the user as an **intermittent** `[Errno 2] No such file or directory` under `/var/triliovault-mounts`, on **S3 targets only** (NFS is a plain kernel mount and never goes through DMS).

`_mask_packaged_units()` handles this from `_install_packages()` — the point where the postinst has just run. All three steps matter: `disable` clears the enablement so it stays off across reboots, `--now` stops the copy the postinst already started (masking does not stop a running unit), and `mask` is the only state a later `apt install`/`apt upgrade` of the Trilio debs cannot silently undo. Note the charm installs *and upgrades* these debs on both the `install` and `upgrade-charm` hooks, so the postinst re-enables them on every charm refresh — a plain `systemctl stop` on a node is never a durable fix. Masking happens in a `finally`, so a part-way apt failure cannot skip it. Diagnose with `systemctl list-unit-files | grep -E 'dms|contego'`; anything other than `masked` for the two packaged units means this regressed — the charm also logs a `Failed to mask package-shipped unit` warning when `systemctl mask` itself fails.
...
### Host device access for the k8s control-plane charms (`_patch_privileged_devices`)
Two different features in `trilio-wlm-k8s` need host devices, and **sidecar k8s charms have no `charmcraft.yaml` field for `securityContext`/host-device access at all** (only `resource` and `mounts` under `containers:`). The charm therefore self-patches its own StatefulSet at runtime via `lightkube` in `_patch_privileged_devices()` — a strategic-merge patch adding `securityContext.privileged: true` plus `dev-fuse`/`lib-modules` hostPath volumes. This requires `trust: true` on the application (already set in `trilio-ctlplane-bundle.yaml`, which grants the operator service account a namespace-scoped `apiGroups: ["*"], resources: ["*"], verbs: ["*"]` Role — no extra RBAC needed). The patch triggers one StatefulSet-driven pod restart to take effect; `_configure()` calls it idempotently so it is a no-op on every subsequent hook run.

**Both workload containers must be patched, for unrelated reasons:**
- `trilio-dms` — `s3vaultfuse` (S3 backup targets) needs `/dev/fuse`. Data-plane machine charms (`trilio-data-mover-sunbeam`, and the legacy `charm-trilio-data-mover`) get this for free: they install `fuse`/`libfuse2` on a real host, which already has `/dev/fuse`.
- `trilio-wlm` — **file search** needs a bind mount of the host `/dev`. See the next section.

**Gotcha — the idempotency guard must be per container.** It originally inspected only `trilio-dms`, so once that container was patched the method returned early and a newly added container was never patched at all. Any future container added to this patch must also be added to the guard, or the change will silently no-op on every existing deployment while working fine on a fresh install.

### File search needs `/dev/nbd*` **inside the `trilio-wlm` container** — the module alone is not enough
File search (`workloadmgr/workloads/filesearch.py`) runs as part of **wlm-workloads**, which on this charm is a Pebble service in the `trilio-wlm` container — there is no separate workloads container; `wlm-api`, `wlm-workloads`, `wlm-scheduler` and `wlm-cron` are all Pebble services in that one container. It attaches each backup qcow2 to an NBD device, and `list_devices()` finds candidates by scanning **the container's own `/dev`** for `nbd*` entries.

This is the trap: `/dev` is per-container, but `lsmod` and `/sys/block` are not. A container can show `nbd` loaded via `lsmod` and 128 entries under `/sys/block/nbd*` while holding **zero** `/dev/nbd*` nodes. Loading the module from the privileged `trilio-dms` sidecar (`_load_nbd_module`) shares the *module*, never the device nodes. When that happens `find_available()` returns `[]`, `connect()` returns the boolean `False`, and the search dies running `lsblk -O -J /dev/False` — surfacing in Horizon as "Invalid state: Error in searching files, Contact your administrator", with the real clue only in `/var/log/triliovault/workloadmgr-filesearch.log`. The `modprobe: FATAL: Module nbd not found` line in that log is a red herring: `filesearch.py` wraps that call in try/except and only logs it.

**`privileged: true` alone does NOT fix this — you need a hostPath bind mount of `/dev`.** A container's `/dev` is a private **tmpfs** that the runtime populates with device nodes *once, at container creation*; it is not the host devtmpfs, even under `privileged`. Confirm for yourself with `mount | grep " /dev "` inside any privileged container here — it reports `tmpfs on /dev type tmpfs`, not `devtmpfs`. Two consequences, both of which produce the reported symptom:
- On a node where `nbd` is loaded only *after* the pod started, the creation-time snapshot contains no `/dev/nbd*` at all, and nothing restarts the pod afterwards.
- The `/dev/nbdXpN` partition nodes `partprobe` creates at runtime never appear, so the per-partition `mount()` calls in `filesearch.py` fail even when the whole-disk attach worked — which is every VM disk with a partition table.

A bind mount is a live view of host devtmpfs, so both resolve themselves and the modprobe-vs-pod-creation ordering stops mattering. `privileged: true` is still required *alongside* it, for the `CAP_SYS_ADMIN` that `qemu-nbd -c`, `partprobe`, the LVM scans and the partition `mount` calls need. This is exactly what Kolla does — `kolla-ansible/ansible/roles/triliovault/defaults/main.yml` gives all four wlm containers `privileged: True` **and** `/dev:/dev` plus `/lib/modules:/lib/modules:ro`. Static hostPath `CharDevice` mounts of individual nbd nodes are not a substitute, for the partition-node reason above.

**Related caveat:** `_load_nbd_module` runs `modprobe nbd nbds_max=128`, but modprobe is a no-op on an already-loaded module — it does **not** re-apply `nbds_max`. On a node where something else loaded `nbd` first the count stays at the default 16, capping how many backup images one file search can attach concurrently. (Observed live: one controller at 16, another at 128.)
...
- **`trilio-dm-api-k8s` now runs as root too** (2026-08-29, TVAULT-7653) — it was originally left out of the root-user change on the grounds that it never touches the shared NFS backup-target filesystem, and kept the `dmapi` Pebble user (uid 63630). That made it the only Trilio service on Sunbeam not running as root, and it earned a bug class of its own (next bullet). The `user`/`group` keys are gone from its Pebble layer; the image's own `USER dmapi` directive was already being overridden by Juju's sidecar packaging, so root was only ever one charm line away. Least privilege on the one network-exposed API (port 8784) is the trade being made here — revisit it if dm-api ever gains untrusted input paths.
- **dm-api log-file ownership — fixed structurally, don't reintroduce the workaround** (TVAULT-7653): while the service ran as `dmapi`, anything that created `/var/log/triliovault/triliovault-datamover-api.log` as root made it unopenable by the service — oslo_log's `FileHandler` raised `PermissionError`, `dmapi-api` exited before it could log anywhere but stdout, and Pebble respawned it in a silent ~30s crash loop while the Juju unit still showed `active`/`idle`. The culprit was `dmapi-dbsync`, which shares `log_config_append` with the service and runs on the **leader only** — so exactly one replica per application died, always the leader, and Kubernetes happily kept it in the Service endpoints refusing roughly a third of every WLM call to DM-API. A 2026-07-25 fix added `_ensure_log_permissions()` (a `chown -R dmapi:dmapi` before the Pebble layer) but it ran *before* `_db_sync()`, so dbsync recreated the file root-owned moments later and the bug survived. The real fix was to stop having two identities in play: dm-api runs as root (previous bullet), so `_ensure_log_permissions()` has been **deleted** — root cannot be locked out of its own files. If you ever put dm-api back under a non-root user, this returns immediately: chown on **both** sides of `_db_sync()`, and do *not* try to fix it by running dbsync as that user instead — on a node already carrying a root-owned log file, dbsync dies on `PermissionError` and takes the whole hook down before any chown can heal it, turning a self-healing state into a wedged one.
...
### Compute nodes lack the Nova runtime contego assumes (TVAULT-7653)
On every other distro the datamover runs *inside* a Nova container, so it inherits Nova's user, its rootwrap, and an rbd-capable qemu for free. On Sunbeam it runs on the bare host next to an `openstack-hypervisor` **snap**, which provides none of them. Verified absent on a pristine Sunbeam compute node (no Trilio installed): `nova-rootwrap`, `/etc/nova/rootwrap.conf`, the `nova` user, `qemu-block-extra`, `/usr/lib/x86_64-linux-gnu/qemu/`. The charm must therefore install `nova-common` + `python3-nova` + `qemu-block-extra` itself. Three things this is easy to get wrong:

- **contego hardcodes `sudo nova-rootwrap <config> rbd ...`** for every rbd call. Missing either half fails as `sudo: nova-rootwrap: command not found`, which contego then reports as `Could not find any valid ceph key to use` / `ceph credentials are not available in nova.conf` — both badly misleading, since the keyring and `ceph.conf` are present and correct. Always check the *datamover* log for the underlying command failure before believing a "ceph credentials" message. `/usr/bin/nova-rootwrap` comes from `python3-nova`, `/etc/nova/rootwrap.conf` from `nova-common` — you need both packages, not one.
- **Ubuntu's qemu is modular.** `/usr/bin/qemu-img` cannot open an `rbd:` URI without `qemu-block-extra` (`Unable to load block driver rbd`). Trilio ships its own rbd-capable build at `/usr/local/bin/trilio/qemu-img`, but contego's unencrypted restore path invokes a bare `qemu-img` off `$PATH` and so bypasses it — a product-side bug; `qemu-block-extra` is the deployment-side workaround. Note the snap's own qemu-img has no rbd either, so neither of the two binaries already on the node is usable.
- **Package install order is load-bearing.** `python3-tvault-contego`'s postinst runs `usermod -a -G disk nova` and `chown -R nova:nova ...` under `set -e` but never creates the `nova` user; `python3-trilio-dms`'s postinst does create it, and **nothing declares a dependency between the two**. In a single `apt-get install` apt picks its own configuration order, so on a clean node the install is a coin flip that fails outright when contego is configured first. `_install_packages()` therefore uses **two separate apt transactions** — host prerequisites, then Trilio packages. Don't merge them back.

Aside on the `nova` uid: `nova-common` pins it to Ubuntu's static **64060**, the same id the WLM and dm-api images get (their Dockerfiles install `nova-common` too). But it only creates the user if absent, so nodes where an earlier charm revision let `python3-trilio-dms` create it keep that arbitrary `--system` uid forever. `_warn_on_nova_uid()` reports the mismatch rather than repairing it — renumbering a live uid means re-chowning mounted backup-target data from a charm hook. Harmless under the current root-user architecture; it only bites if a service is ever put back under the nova user.
...
#### `/etc/ceph` is shared with the host — only ever create, never clobber
The datamover is co-located with nova-compute on the host, so host `/etc/ceph` is not automatically ours. It usually looks empty because the hypervisor is a **snap** and keeps its Ceph config under `/var/snap/openstack-hypervisor/common/etc/` (microceph likewise under `/var/snap/microceph/`) — the host directory is typically created only by the `ceph-common` deb the charm itself installs. But an operator, or a deb-based Ceph client, can legitimately have put a real cluster config there.

`_ensure_ceph_conf()` therefore writes `/etc/ceph/ceph.conf` **only if the charm owns it**, tracked by the sentinel `/etc/ceph/.trilio-managed-ceph-conf`. An unmarked pre-existing conf is left strictly alone — contents, mode and ownership. A marked one is rewritten whenever the rendered content differs, so it can't go stale if microceph's monitors move (this also means hand edits to a charm-owned conf get reverted — the file is ours, and there's no sane merge for a regenerated template).

Four non-obvious details, each of which was a real bug in the first cut of this fix:
- **Ownership is inferred, not just marked.** A conf written by a charm revision *predating* the marker has no marker, so on `juju refresh` it would be misread as an operator's file and disowned permanently — the charm could then never refresh its own mon list, which is precisely what the marker exists to prevent. `_is_charm_shaped_conf()` adopts a conf whose content matches the charm's template exactly (`[global]` + `mon host` + the three cephx lines, nothing else). Keep it strict: a genuine operator conf always carries an fsid, tuning keys or per-`[client.x]` sections.
- **Write the marker before the conf.** A hook killed between the two writes leaves either a marker with no conf (self-corrects next hook) or a conf with no marker (disowned forever). Only the first ordering is recoverable.
- **`os.path.lexists()`, not `exists()`.** `exists()` follows symlinks, so a *dangling* `/etc/ceph/ceph.conf` symlink would read as absent and the write would land on whatever it points at — the same clobbering, one indirection away.
- **Remove the marker on `stop`.** Otherwise it outlives `juju remove-application`, and a redeploy would treat a conf written by an operator in the meantime as its own.

Two standing constraints when touching this code:
- The charm has **no `os.chown` anywhere** and never deletes from `/etc/ceph`. `open(path, "w")` on an existing file preserves owner:group and only rewrites contents, so a stray `chmod` is the whole permission story — that's why the `0644` on `ceph.conf` is applied on create only.
- The keyring is the one file the charm genuinely owns and is always refreshed to the latest relation key. It is safe to overwrite unconditionally *because* it is named after the Juju application (`ceph.client.<app>.keyring`) and so cannot collide with `cinder-ceph`'s or `nova-compute`'s. It is `0640 root:root`, which only works because the datamover runs as **root** — reverting to `User=nova` would need a chown or group-read here.

**Known gap (not fixed here):** when a foreign conf is found the charm logs a warning and continues, but `triliovault-datamover.conf` still points `images_rbd_ceph_conf` at `/etc/ceph/ceph.conf` and `_ceph_backend_enabled()` is driven by the pool-grant marker, not by conf ownership. If that foreign conf describes a *different* Ceph cluster, contego is silently configured against it with our microceph keyring and rbd reads fail at backup time, hours later. The clean fix is a charm-owned conf path (e.g. `/etc/triliovault-datamover/ceph.conf`, with an explicit `keyring =` line) since `ceph_conf_path` is already a template variable — deliberately deferred to keep the deployed layout unchanged.
...
This rule replaces the earlier "always pin reference entries" guidance, which was wrong in both directions and caused two separate incidents:

- **Unpinned (2026-07-24):** omitting `revision:`/`scale:` made `juju deploy` force-scale all five ctlplane reference apps toward 0 units and force-upgrade their revisions to whatever the channel resolved to — a near-total control-plane outage (nova, neutron, cinder, glance, horizon, barbican, placement all blocked/error, plus ~35 mysql-router subordinates). ~20 minutes of `juju resolved` cycling to recover; no data lost.
- **Pinned (TVAULT-7644):** pinning is only correct for the one cloud the numbers were read off. `microceph` was pinned to revision 251 from the dev cloud; the QA cloud runs 296, so the same file was an instruction to **downgrade Sunbeam's storage layer**. Pinning does not fix the problem, it relocates it to every other cloud — and to the next time Sunbeam itself is upgraded.

There is no safe value for these fields, because the bundle cannot know what any given cloud is running. The only safe bundle is one that never mentions an application it does not own. The same applies to the `saas:` block: offer URLs embed the controller, user and model name, so `deploy_trilio.py` reads the real URLs out of the control-plane model rather than hardcoding one lab's.

`juju deploy ./bundle.yaml --dry-run` remains a good pre-flight check: it must show ONLY Trilio applications, and never "scale X to N units" or "upgrade charm" for anything else.

### `[cinder] http_retries` is a **nova** option, not a contego one
`grep http_retries` in the installed contego package finds nothing, which makes it look dead — it isn't. Nova registers the option and reads it in `nova/volume/cinder.py` as the Cinder client's `connect_retries`; contego reuses Nova's Cinder API wrapper and is started with **both** `nova.conf` and `triliovault-datamover.conf` (see the systemd `ExecStart`), so the value set in the datamover conf wins for the datamover process. Every other distro sets 10 (kolla-ansible, RHOSP17, RHOSO18, openstack-helm, the legacy Juju charm); Sunbeam was the only one leaving it unset and silently running on nova's much lower default. The general lesson: before concluding a datamover conf key is inert, grep **nova** too — the two config files share one namespace at runtime.
