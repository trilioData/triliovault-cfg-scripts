...
- **Never flip a resource between hook and non-hook across releases (TVAULT-7556)**: Helm hook resources are excluded from the release manifest and are never stamped with Helm ownership metadata (`app.kubernetes.io/managed-by: Helm`, `meta.helm.sh/release-name`, `meta.helm.sh/release-namespace`). So when TVAULT-7517 de-hooked `rabbitmq-cluster.yaml`, the next upgrade tried to adopt the already-existing CR as a regular manifest resource and Helm refused: `failed to get candidate release: ... invalid ownership metadata; label validation error: missing key "app.kubernetes.io/managed-by"`. This aborts **before a candidate release is even built**, so no hooks run and no Deployments are updated - the visible symptom is "control plane configured but pods still on the old image", with the release stuck at its previous revision. Reproduced end-to-end on the FR5 setup upgrading 6.2.0 -> 6.2.1-maint-7.2 (release stayed at revision 1, all pods on 6.2.0). Fixed by restoring the hook annotation as **`helm.sh/hook: post-install` only** (never `post-upgrade`): the CR stays hook-managed so the ownership check never applies, and because `helm upgrade` fires only pre/post-upgrade hooks, the existing CR is never re-run, so it is not deleted+recreated either (which is what TVAULT-7517 was about). Keep `helm.sh/hook-weight: "-18"` - it orders the CR ahead of the rabbitmq wait-job (`-17`) and rabbitmq-init Jobs (`-4`) on a fresh install; without it the weight defaults to 0 and the wait-job blocks on a cluster that does not exist yet. Do **not** add a `hook-delete-policy` of `hook-succeeded` (it would delete the broker after install). Two consequences to remember: (a) the CR spec is frozen after install, so RabbitMQ fields in `tvo-operator-inputs.yaml` are no longer propagated by an upgrade - any future CR change needs a migration script; (b) a release installed while the CR *was* manifest-managed (i.e. an intermediate 6.2.1 build) has it in the old manifest but not the new one, and Helm deletes resources dropped from the manifest - such setups need a reinstall rather than an in-place upgrade.
