...
- **Galera CR name must never contain the substring `-galera-`**: RHOSO's mariadb-operator (`report_local_galera_state.py`, from `registry.redhat.io/rhoso/openstack-mariadb-rhel9`) reverse-derives the Galera CR name from the pod hostname via an unanchored `re.sub(r'-galera-[0-9]*', '', pod_name)`. This strips *every* occurrence of `-galera-`, not just the trailing `-galera-<ordinal>` StatefulSet suffix — so a CR name with `-galera-` anywhere else in it (e.g. the old `trilio-galera-cluster`) gets mangled, the operator 404s writing that (non-existent) CR's status, and Galera pods hang forever at "Waiting for gcomm URI" (TVAULT-7493). The CR is named `trilio-db-cluster` in `templates/galera/galera-cluster.yaml` for this reason — don't rename it back to something containing `-galera-`. The pod name (`<CR-name>-galera-<ordinal>`) is hardcoded in two other places that must stay in sync with the CR name: `galera-cluster-wait-job.yaml`'s `POD_NAME` prefix, and `service-galera-lb`'s selector. The consumer-facing DNS name (`trilio-galera-cluster.trilio-openstack.svc`, set via a `DNSData` resource in `job-galera-create-dns-entry.yaml` pointing at the `trilio-galera-lb` LoadBalancer Service) and the TLS cert/secret names (`cert-trilio-galera-cluster`) are independent of the CR name and were deliberately left unchanged — they're custom resources, not generated by the operator from the CR name.
- **Galera pod label key is `service`, not `app.kubernetes.io/name`**: RHOSO's Galera controller (`internal/mariadb/utils.go` `StatefulSetLabels`/`StatefulSetName`, verified against the `18.0-fr6` branch) labels every Galera pod and its StatefulSet's own selector with `service: <CR-name>-galera` (`common.AppSelector` from `lib-common` is literally the string `"service"`), plus `app: galera`, `cr: galera-<CR-name>`, and three `galera.openstack.org/{uid,namespace,name}` ownership labels. It never sets `app.kubernetes.io/name` anywhere — confirmed by grepping the whole operator codebase. `service-galera-lb`'s selector must use `service: <CR-name>-galera` to actually match pods (TVAULT-7493, PR #1527); an `app.kubernetes.io/name`-based selector silently matches zero pods regardless of CR naming.
