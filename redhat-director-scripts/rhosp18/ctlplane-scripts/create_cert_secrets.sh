...
if ! oc get secret "$SECRET_NAME" -n "$SOURCE_NAMESPACE" > /dev/null 2>&1; then
...
INTERNAL_CA_BUNDLE=$(oc get secret "$SECRET_NAME" -n "$SOURCE_NAMESPACE" -o jsonpath='{.data.internal-ca-bundle\.pem}' | base64 -d)
TLS_CA_BUNDLE=$(oc get secret "$SECRET_NAME" -n "$SOURCE_NAMESPACE" -o jsonpath='{.data.tls-ca-bundle\.pem}' | base64 -d)
...
oc create secret generic "$SECRET_NAME" -n "$TARGET_NAMESPACE" \
