...
echo "rabbit_transient_quorum_queue is enabled - will converge '*_fanout' exchanges in vhost '${WLMAPI_RABBITMQ_VHOST_NAME}' to durable=true."
...
if [ "$api_ready" -ne 1 ]; then
  ...
  echo "WARNING: the upgrade continues; wlm pods may crash-loop until RabbitMQ auto-deletes the stale exchange (pre-TVAULT-7572 behaviour)."
  exit 0
fi

# This runs as a post-upgrade hook at weight 99, i.e. immediately before this release's
# Deployments (they are themselves post-upgrade hooks at weight 100-104). So the vhost
...
#
# Scoped narrowly on purpose: only a *_fanout exchange that is type=fanout and whose durable
# flag disagrees with rabbit_transient_quorum_queue is touched - every other exchange
# (already durable=true, not a *_fanout name, or not type fanout) is left untouched, and this
# whole script is a no-op unless rabbit_transient_quorum_queue is enabled. auto_delete is
# preserved as observed rather than forced, because forcing a value oslo.messaging does not
# declare would just move the PRECONDITION_FAILED onto auto_delete instead. Queues are
# intentionally NOT touched here, even though the same bug leaks orphaned per-consumer queues
# under concurrent pod startup - that cleanup is out of scope for this fix.
...
rmqadmin() {
  rabbitmqadmin -H "$RABBITMQ_HOST" -P "$RABBITMQ_PORT" \
    ...
    -V "${WLMAPI_RABBITMQ_VHOST_NAME}" $RABBITMQADMIN_EXTRA_ARGS "$@"
}
...
for pass in $(seq 1 "$MAX_PASSES"); do
  ...
  if ! exchanges="$(rmqadmin list exchanges name type durable auto_delete -f tsv)"; then
    echo "WARNING: could not list exchanges in vhost '${WLMAPI_RABBITMQ_VHOST_NAME}' (pass ${pass}/${MAX_PASSES})."
    sleep "$PASS_INTERVAL"
    continue
  fi
  ...
  while IFS=$'\t' read -r name type durable auto_delete; do
    ...
    echo "DELETING stale non-durable fanout exchange '${name}' (type=${type}, durable=${durable}, auto_delete=${auto_delete}) in vhost '${WLMAPI_RABBITMQ_VHOST_NAME}'"
  ...
  echo "PASS ${pass}/${MAX_PASSES}: vhost '${WLMAPI_RABBITMQ_VHOST_NAME}' - ${examined} '*_fanout' exchange(s) examined, ${mismatched} mismatched, ${fixed} converged, ${failed} failed."
...
if [ "$converged" -eq 1 ]; then
  echo "SUMMARY: vhost '${WLMAPI_RABBITMQ_VHOST_NAME}' - CONVERGED after ${converged_pass} pass(es); every '*_fanout' exchange is now durable=true."
else
  echo "WARNING: vhost '${WLMAPI_RABBITMQ_VHOST_NAME}' - NOT CONVERGED after ${MAX_PASSES} passes; a non-durable '*_fanout' exchange is still being re-created by an old client."
  echo "WARNING: exiting 0 so the upgrade is not blocked - wlm pods may crash-loop until RabbitMQ auto-deletes the stale exchange (pre-TVAULT-7572 behaviour)."
fi
exit 0
