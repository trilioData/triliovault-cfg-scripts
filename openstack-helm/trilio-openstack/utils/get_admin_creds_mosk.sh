...
if [ $# -lt 2 ];then
   ...
   echo -e "./get_admin_creds_mosk.sh <internal_domain_name> <public_domain_name>"
   echo -e "./get_admin_creds_mosk.sh cluster.local company.public.net"
   exit 1
fi
...
CA=$(kubectl -n openstack get secrets/keystone-tls-public -o "jsonpath={.data['ca\.crt']}" | base64 -d | sed 's/^[ \t]*//' | sed 's/^/            /') 
cd ../
