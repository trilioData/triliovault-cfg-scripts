...
## Cloud admin trust — TVAULT-7484
- **The chart creates the cloud admin trust; it is not an operator step.** `manifests.job_wlm_cloud_trust` shipped as `false`, so on a stock install the `post-install,post-upgrade` hook in `templates/job-wlm-cloud-trust.yaml` never rendered and the trust was never created. Every other distro creates it at deploy time (rhosp17 as a puppet-rendered `step_5` container, rhoso18 via the same Helm hook, enabled). The flag now defaults to `true`; do not flip it back.
- **The ordering is trust → backup target, not the reverse.** `scripts/upgrade-process.md` used to justify disabling the job with "Trust Creation uploads metadata to the storage backend, so it must run after the Backup Target is online". No code in the repo supports that. The real dependency is the other way round and is documented in `test/04_create_backup_targets.sh`: `backup-target-create` performs a trust lookup and fails with `No cloud admin trust found. Please recreate using CLI` if the trust is missing — which is why `test/run_all.sh` runs trust (step 03) before targets (step 04). The 6.2 chart never creates a backup target at all (they are registered at runtime via the CLI), so a job that waited for one would deadlock on a fresh install.
- **`workloadmgr trust-create` exits 0 even when wlm-api answers HTTP 500** — the cliff/cmd2 layer swallows it. The only proof a trust exists is a non-empty `trust-list`, which is why `bin/_triliovault-wlm-cloud-trust.sh.tpl` verifies before exiting instead of trusting the return code.
- **Under `set -e`, `var=$(cmd)` aborts the script the moment `cmd` fails.** That is what made the trust script's retry loop dead code for every hard failure — the `status=$?` line after it was never reached, and neither was the `sleep 30s; continue` branch. (`status=$?` itself is fine: after a pure assignment it *does* carry the command substitution's status. The bug was `set -e`, not the status read.) Wrapping the call as `if command_output=$(...); then` suspends errexit for that command and makes the loop reachable.
- **`trust-list` needs `--is_cloud_admin True`, and its absence is not cosmetic.** A cloud trust is stored with `user_id='cloud_admin'` (a literal string, not a UUID), while `trust_list` filters on `context.user_id` unless that flag is set — so a plain `trust-list` compares the admin's real UUID against `'cloud_admin'` and returns empty on a perfectly healthy deployment. Note the flag is `--is_cloud_admin` on `trust-list` but `--is_cloud_trust` on `trust-create`; they are not interchangeable and each errors on the other's name.
- **Two independent ordering mechanisms, and both matter.** The chart renders 13 `helm.sh/hook` resources at weights `-5`..`-1` (db-init, db-sync, rabbit-init, ks-*), plus the trust job at `1` — the highest, so it runs last among hooks. Do not treat that weight as decorative. Layered on top, the `kubernetes-entrypoint` init container driven by `dependencies.static.wlm_cloud_trust` in `values.yaml` blocks until db-sync, rabbit-init, ks-endpoints and ks-user have succeeded and the `workloads` internal endpoint is ready. Neither mechanism can express "backup target is ready": `kubernetes-entrypoint` only understands Kubernetes objects, and a backup target is a DB row plus a DMS mount with no Kubernetes representation.
- **Helm blocks on the hook, so the install needs a `--timeout`.** The retry loop alone can run 10 x 30s, which meets or exceeds helm's 5m default and would fail the release even on a run that eventually succeeds. `utils/install.sh`, `install_mosk.sh`, `upgrade.sh` and `upgrade6.2.sh` all pass `--timeout 20m` for this reason.
- **Helm does not garbage-collect hook resources on `helm uninstall`**, so `utils/uninstall.sh` must delete the `triliovault-wlm-cloud-trust` Job explicitly. Re-install and upgrade are already safe: with no `helm.sh/hook-delete-policy` set, Helm 3's default `before-hook-creation` applies.
